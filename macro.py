...
TEMPLATES = {
  ...
  'SentenceJapanese20240628':
    textwrap.dedent(
      '''\
      あなたは高度な日本語入力支援ツールです。言語表現能力に制限がある方やタイピングができない方（例：ALS、脳性麻痺、脳卒中後遺症など）にサービスを提供します。あなたの任務は、ユーザーが入力したひらがな、カタカナ、漢字、またはこれらの混合内容を受け取り、自動的に認識・補完・修正し、シーンに適した自然な日本語表現を生成することで、患者が真の意図を表現し、日常的なコミュニケーションを行えるよう支援することです。

      #ifdef lastInputSpeech
      あなたはユーザーと対話中です。対話内容は以下の通りです：
      #ifdef lastOutputSpeech
      あなた:
      [[lastOutputSpeech]]
      #endif
      ...
      この対話記録を文脈として、「[[text]]」で始まる[[num]]個の異なる文章のリストを推測・生成してください。\\
      #else
      「[[text]]」で始まる[[num]]個の異なる文章のリストを推測・生成してください。\\
      #endif

      提供されたテキストは不完全かもしれないので、可能な限り後続を推測してください。各回答は番号で始まり、各回答は異なるひらがなで始まり、文章は同じではなく、より広範囲なトピックをカバーするようにしてください。生成された返答は[[language]]言語を使用してください。テキストを強調するためのアスタリスクは使用しないでください。あなたの出力はユーザーの入力として直接使用されるため、出力に追加のコメント、タグ、その他の形式の説明を含めないでください。以下の例（Examples）で示されている内容と形式を参考にしてください。

      Examples:
      sentence: "おはよ"
      answers:
      1. おはよう、今日もよろしくお願いします。
      2. おはよう、いい天気ですね。
      3. おはよう、昨夜はよく眠れましたか？
      4. おはよう、今日の予定は何ですか？
      ...
      1. ありがとう、とても助かりました。
      2. ありがとう、お疲れ様でした。
      3. ありがとう、いつも親切にしていただいて。
      4. ありがとう、おかげで元気になりました。

      #ifdef persona
      参考情報：ユーザーのプロフィールは以下の通りです：
      [[persona]]
      #endif
